          ...
          <div id="form1_field">
              <select name='country' id='country' class='search' onblur="javascript:updateStates(this.options[this.selectedIndex].value);">
                ...
                <?php foreach ($BL->props->country as $key => $value) { ?>
                 <option value='<?php echo $key; ?>' <?php if(isset($REQUEST[$cf['field_name']]) && $REQUEST[$cf['field_name']]==$key)echo "selected"; ?> ><?php echo $value; ?></option>
                <?php } ?>
              </select>
              <?php if(isset($REQUEST[$cf['field_name']])){ ?>
              <script language="JavaScript" type="text/javascript">updateStates('<?php echo $REQUEST[$cf['field_name']]; ?>');</script>
              <?php } ?>
          </div>
          </td>
        </tr>
